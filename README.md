...
* `--encrypt` - шифрование файла при помощи `--encrypt-cmd` перед операцией загрузки (put) в хранилище;
* `--decrypt` - расшифровка файла при помощи `--decrypt-cmd` после операции получения (get) из хранилища;
